...
std::pair<cl_platform_id, cl_device_id> findDevice() {
    ...
    // Ищем наиболее мощные ГПУ и ЦПУ девайсы
    cl_platform_id platformGPUId = nullptr;
    cl_device_id deviceGPUId = nullptr;
    cl_uint freqGPU = 0;

    cl_platform_id platformCPUId = nullptr;
    cl_device_id deviceCPUId = nullptr;
    cl_uint freqCPU = 0;
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            switch (deviceType) {
                case CL_DEVICE_TYPE_CPU:
                    if (freq > freqCPU) {
                        platformCPUId = platformId;
                        deviceCPUId = deviceId;
                        freqCPU = freq;
                    }
                    break;
                case CL_DEVICE_TYPE_GPU:
                    if (freq > freqGPU) {
                        platformGPUId = platformId;
                        deviceGPUId = deviceId;
                        freqGPU = freq;
                    }
                    break;
                default:
    ...
    if (deviceGPUId) {
        return std::make_pair(platformGPUId, deviceGPUId);
    }
    else {
        return std::make_pair(platformCPUId, deviceCPUId);
    }
}
...
int main()
{
    ...
    auto platformDevicePair = findDevice();
    cl_platform_id platformId = platformDevicePair.first;
    cl_device_id deviceId = platformDevicePair.second;
    ...
    cl_int errCode;
    std::vector<cl_context_properties> contextProperties{CL_CONTEXT_PLATFORM, (cl_context_properties)platformDevicePair.first, 0};
    auto context = clCreateContext(contextProperties.data(), 1, &platformDevicePair.second, nullptr, nullptr, &errCode);
    ...
    auto queue = clCreateCommandQueue(context, platformDevicePair.second, 0, &errCode);
    OCL_SAFE_CALL(errCode);

    unsigned int n = 100U*1000*1000;
    ...
    auto as_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), as.data(), &errCode);
    OCL_SAFE_CALL(errCode);
    auto bs_gpu = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, n * sizeof(float), bs.data(), &errCode);
    OCL_SAFE_CALL(errCode);
    auto cs_gpu = clCreateBuffer(context, CL_MEM_WRITE_ONLY, n * sizeof(float), nullptr, &errCode);
    OCL_SAFE_CALL(errCode);
...
//         std::cout << kernel_sources << std::endl;
    ...
    auto program = clCreateProgramWithSource(context, 1, &kernel_sources_ptr, nullptr, &errCode);
    OCL_SAFE_CALL(errCode);
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            cl_event event;
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &globalWorkSize, &workGroupSize, 0, nullptr, &event));
        ...
        std::cout << "GFlops: " << n / t.lapAvg() / std::pow(10.0, 9.0) << std::endl;
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, cs_gpu, true, 0, n * sizeof (float), cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
